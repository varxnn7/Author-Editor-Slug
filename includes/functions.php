<?php
/**
 * Author Slug Editor Core Functions
 *
 ...
 */
function vk_ase_do_auto_update() {
	 ...
	 */
	return (bool) apply_filters( 'vk_ase_do_auto_update', vk_ase()->do_auto_update );
}
 ...
 */
function vk_ase_do_bulk_update( $do_bulk = false ) {
	...
	/**
	 * Filters the return of the `vk_ase_do_bulk_update()`.
	 *
	 * @since 1.4.0
	 *
	 * @param bool $retval The `do_auto_update` option.
	 */
	return (bool) apply_filters( 'vk_ase_do_bulk_update', $retval );
}
 ...
 */
function vk_ase_auto_update_user_nicename( $user_id = 0, $bulk = false, $structure = '' ) {
	...
	if ( false === $bulk && ! vk_ase_do_auto_update() ) {
		return false;
	}
	 ...
	 */
	$structure = apply_filters( 'vk_ase_auto_update_user_nicename_structure', $structure, $user_id );

	// Setup the current nicename.
	$old_nicename = $user->user_nicename;

	// Sanitize and trim the new user nicename.
	$nicename = vk_ase_get_nicename_by_structure( $user_id, $structure );
	 ...
	 * @param string $structure The auto-update structure.
	 */
	$nicename = apply_filters( 'vk_ase_pre_auto_update_user_nicename', $nicename, $user_id, $structure );
 ...
 */
function vk_ase_auto_update_user_nicename_bulk( $do_bulk = false ) {

	// Bail if the user didn't ask to run the bulk update.
	if ( ! vk_ase_do_bulk_update( $do_bulk ) ) {
		return false;
	}

	// Nonce check.
	check_admin_referer( 'author-slug-editor-options' );
	...
	if ( isset( $_POST['_vk_ase_bulk_update_structure'] ) ) {
		$structure = sanitize_key( $_POST['_vk_ase_bulk_update_structure'] );
	}
	 ...
	 */
	$users = (array) apply_filters( 'vk_ase_auto_update_user_nicename_bulk_user_ids', $users );
	...
	foreach ( $users as $key => $user_id ) {
		...
		$user     = get_user_by( 'id', $user_id );
		$nicename = vk_ase_get_nicename_by_structure( $user->ID, $structure );
		$exists   = vk_ase_nicename_exists( $nicename, $user );
 ...
 */
function vk_ase_sanitize_nicename( $nicename = '', $strict = true ) {
 ...
 * @since 1.3.0 Allow `%vk_ase_author_role%` rewrite tag in author base.
 *
 * @param string $author_base Author base to be sanitized.
 *
 * @return string The author base.
 */
function vk_ase_sanitize_author_base( $author_base = 'author' ) {
	...
	if ( ! empty( $author_base ) && 'author' !== $author_base ) {
		...
		// Sanitize all parts except our rewrite tag, `%vk_ase_author_role%`.
		foreach ( $parts as $key => $part ) {

			if ( '%vk_ase_author_role%' !== $part ) {
	 ...
	 */
	return apply_filters( 'vk_ase_sanitize_author_base', $author_base, $original_author_base );
}
 ...
 * @return string The nicename.
 */
function vk_ase_esc_nicename( $nicename = '' ) {
 ...
 * @return string The nicename.
 */
function vk_ase_trim_nicename( $nicename = '' ) {
 ...
 */
function vk_ase_nicename_is_ascii( $nicename = '' ) {
	$sanitize_nicename        = vk_ase_sanitize_nicename( $nicename );
	$sanitize_nicename_strict = vk_ase_sanitize_nicename( $nicename, false );
 ...
 */
function vk_ase_get_nicename_by_structure( $user_id = 0, $structure = '' ) {
	...
	// Sanitize and trim the new user nicename.
	$nicename = vk_ase_trim_nicename( vk_ase_sanitize_nicename( $nicename ) );

	/**
	 * Filters the return of `vk_ase_get_nicename_by_structure`.
	 *
	 * @since 1.4.0
	 *
	 ...
	 */
	return apply_filters( 'vk_ase_get_nicename_by_structure', $nicename, $user_id, $structure );
}
 ...
 */
function vk_ase_nicename_exists( $nicename = '', $user_or_id = 0 ) {
	...
	/**
	 * Filters the return of `vk_ase_nicename_exists()`.
	 *
	 * @since 1.5.0
	 *
	 ...
	 * @param int|WP_User  $user_or_id The user id or user object.
	 */
	return apply_filters( 'vk_ase_nicename_exists', $retval, $nicename, $user_or_id );
}
...
function vk_ase_wp_rewrite_overrides() {
	...
	if ( $role_based && false === strpos( $author_base, '%vk_ase_author_role%' ) ) {
		$author_base = '%vk_ase_author_role%';
	}
 ...
 * @return bool
 */
function vk_ase_remove_front() {

	$reval = ( vk_ase_has_front() && vk_ase()->remove_front );
	 ...
	 */
	return (bool) apply_filters( 'vk_ase_remove_front', $reval );
}
 ...
 * @return bool
 */
function vk_ase_has_front() {

	$retval = ( '/' !== $GLOBALS['wp_rewrite']->front );

	/**
	 * Filters the return of the `vk_ase_has_front` option.
	 *
	 * @since 1.2.0
	 *
	 * @param bool $has_front The `remove_front` option.
	 */
	return (bool) apply_filters( 'vk_ase_has_front', $retval );
}
 ...
 */
function vk_ase_do_role_based_author_base() {
	 ...
	 */
	return (bool) apply_filters( 'vk_ase_do_role_based_author_base', vk_ase()->do_role_based );
}
 ...
 */
function vk_ase_author_link( $link = '', $user_id = 0 ) {

	// Add a role slug if we're doing role based author bases.
	if ( vk_ase_do_role_based_author_base() && false !== strpos( $link, '%vk_ase_author_role%' ) ) {

		// Setup the user.
		$user = get_userdata( $user_id );

		// Grab the first listed role.
		$role = vk_ase_get_user_role( $user->roles, $user_id );

		// Make sure we have a valid slug.
		$slug = isset( vk_ase()->role_slugs[ $role ]['slug'] ) ? vk_ase()->role_slugs[ $role ]['slug'] : '';
		$slug = empty( $slug ) ? vk_ase()->author_base : $slug;

		// Add the role slug to the link.
		$link = str_replace( '%vk_ase_author_role%', $slug, $link );
	}
 ...
 * @return string Author archive link.
 */
function vk_ase_template_include( $template ) {
	...
	if ( false === $nicename_template && false === $id_template ) {
		...
		// Grab the first listed role.
		$role = vk_ase_get_user_role( $author->roles, $author->ID );
 ...
 * @since 0.9.5
 */
function vk_ase_flush_rewrite_rules() {
 ...
 */
function vk_ase_author_rewrite_rules( $author_rewrite_rules ) {
 ...
 */
function vk_ase_get_user_role( $roles = array(), $user_id = 0 ) {
	 ...
	 * @param int    $user_id The user id.
	 */
	return apply_filters( 'vk_ase_get_user_role', $role, $user_id );
}
 ...
 */
function vk_ase_get_default_role_slugs() {

	// Get the array of WP roles.
	$roles = vk_ase_get_roles();
